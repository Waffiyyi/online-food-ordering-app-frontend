    ...
    <>
      { auth.user && <Card className={'w-[18rem]'}>
        <div
          ...
          className={`${auth.user?.role === "ROLE_CUSTOMER" && item.open ? 'cursor-pointer' : 'cursor-not-allowed'} relative`}
        >
          <img
        ...
        <div className={'p-4 textPort lg:flex w-full justify-between'}>
          <div className={'space-y-1'}>
            <p
              onClick={handleNavigateToRestaurant}
              className={`${auth.user?.role === "ROLE_CUSTOMER" && item.open ? 'cursor-pointer' : 'cursor-not-allowed'} font-semibold text-lg `}
